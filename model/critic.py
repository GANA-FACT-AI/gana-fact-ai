...
class Critic(nn.Module):
    def __init__(self, k):
        super().__init__()
        layers = list()
        layers.append(nn.Conv2d(384, 256, 3))
        layers.append(nn.LeakyReLU(0.2))
        layers.append(nn.MaxPool2d(2))

        layers.append(make_layers(LayerNormBlock, 256, 256, input_size=[1, 64, 8, 8], num_blocks=1, stride=1))

        layers.append(nn.Conv2d(256, 128, 3))
