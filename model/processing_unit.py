...
class ProcessingUnit(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = ComplexConv2d(in_channels=6, out_channels=16, kernel_size=5, stride=1)
        self.tanh = nn.Tanh()
        self.avgpool2d = nn.AvgPool2d(kernel_size=2)
        self.conv2 = ComplexConv2d(in_channels=16, out_channels=120, kernel_size=5, stride=1)
